...
class DebugWeeklyReportView(MailPreviewView):
    def get_context(self, request):
        ...
        timestamp = to_timestamp(
            floor_to_utc_day(
                to_datetime(
                    secrets.SystemRandom().randint(to_timestamp(datetime(2015, 6, 1, 0, 0, 0, tzinfo=timezone.utc)),
        ...
        daily_maximum = secrets.SystemRandom().randint(1000, 10000)

        # Initialize projects
        for i in range(0, secrets.SystemRandom().randint(1, 8)):
            name = " ".join(secrets.SystemRandom().sample(loremipsum.words, secrets.SystemRandom().randint(1, 4)))
            project = Project(
                id=i,
                organization=organization,
                slug=slugify(name),
                name=name,
                date_added=ctx.start - timedelta(days=secrets.SystemRandom().randint(0, 120)),
            )
            project_context = ProjectContext(project)
            project_context.error_count_by_day = {
                start_timestamp + (i * ONE_DAY): secrets.SystemRandom().randint(0, daily_maximum)
                for i in range(0, 7)
            }
            ...
            project_context.dropped_error_count = int(
                secrets.SystemRandom().weibullvariate(5, 1) * secrets.SystemRandom().paretovariate(0.2)
            )
            ...
            project_context.key_errors = [
                (g, None, secrets.SystemRandom().randint(0, 1000)) for g in Group.objects.all()[:3]
            ]

            project_context.new_substatus_count = secrets.SystemRandom().randint(5, 200)
            project_context.escalating_substatus_count = secrets.SystemRandom().randint(5, 200)
            project_context.regression_substatus_count = secrets.SystemRandom().randint(5, 200)
            project_context.ongoing_substatus_count = secrets.SystemRandom().randint(20, 3000)
            ...
            project_context.key_transactions = [
                (
                    f"/test/transaction{secrets.SystemRandom().randint(0, 3)}",
                    secrets.SystemRandom().randint(0, 1000),
                    secrets.SystemRandom().random() * 100,
                    secrets.SystemRandom().randint(0, 1000),
                    secrets.SystemRandom().random() * 100,
                )
                for _ in range(0, 3)
            ...
            project_context.key_performance_issues = [
                (g, None, secrets.SystemRandom().randint(0, 1000))
